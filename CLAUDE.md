...
This project uses Nix for reproducible builds and development. Tailwind and Esbuild binaries are provided through Nix dependencies in `deps.nix`, which prevents download attempts during build.
...
# The Nix shell will load automatically when you cd into the directory
```
...
- The `deps.nix` file is auto-generated by the `deps_nix` package (see `mix.exs` aliases)
- The `flake.nix` file applies overrides to inject Nix-provided `tailwindcss` and `esbuild` binaries
- During build, these binaries are symlinked into the Mix dependency directories
- This prevents the Elixir packages from attempting to download binaries over HTTPS
- Works transparently in both development and production builds

### Updating dependencies

When you update Mix dependencies, regenerate `deps.nix`:

```bash
mix deps.get       # Fetches deps and regenerates deps.nix
# or
mix deps.update    # Updates and regenerates deps.nix
```

The tailwind/esbuild overrides in `flake.nix` will automatically apply to the regenerated file.
