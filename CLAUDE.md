...
# Release
MIX_ENV=prod mix release           # build production release with tar
```

The release creates a self-contained package at `_build/prod/rel/ddbm/` and a tarball at `_build/prod/ddbm-0.1.0.tar.gz`. To run migrations in production:

```bash
bin/ddbm eval "Ddbm.Release.migrate()"
```
